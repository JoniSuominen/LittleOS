...
static struct dirent *initrd_readdir(fs_node_t *node, uint32_t index) {
    if (node == initrd_root && index == 0) {
        strcpy((uint8_t *)dirent.name, "dev");
        dirent.name[3] = 0;
        dirent.ino = 0;
        return &dirent; 
    }
    ...
    strcpy((uint8_t *)dirent.name, root_nodes[index-1].name);
...
fs_node_t *initialize_initrd(uint32_t location) {
    ...
    strcpy((uint8_t *)initrd_root->name, "initrd");
    ...
    strcpy((uint8_t *)initrd_dev->name, "dev");
    ...
    for (uint32_t i = 0; i < initrd_header->nfiles; i++) {
        ...
        strcpy((uint8_t *)root_nodes[i].name, (char *) &file_headers[i].name);
